...
const state={preferences:{locale:'ko',theme:'light',region:'korea',categories:[],keywords:[],exclude:[]},user:null,tab:'today',period:'day',category:'all',q:'',page:1,radar:false,libraryKind:'follow',expanded:false,stories:new Map(),bookmarks:[],feedback:[],hidden:[],version:0};
...
let toastTimer,searchTimer,dialogTrigger;
...
async function load({keep=false}={}){disposePersonalAI?.();disposePersonalAI=null;const version=++state.version;state.tab=tabs[location.hash.slice(1)]?location.hash.slice(1):'today';if(location.hash&&location.hash!=='#main'&&!tabs[location.hash.slice(1)])history.replaceState(null,'','#today');shell();$('#main').setAttribute('aria-busy','true');const more=$('#load-more');if(more)more.disabled=true;if(state.tab==='today')return loadHome(version);if(!keep)$('#main').innerHTML=loader();try{let html;
  if(state.tab==='explore'){const q=scope();q.set('q',state.q);q.set('category',state.category);q.set('page',state.page);html=renderExplore(await request('/api/v2/explore?'+q));}
  else if(state.tab==='library')html=renderLibrary(await request('/api/v2/library'));
  else if(state.tab==='history')html=renderHistory(await request('/api/v2/history'));
...
window.addEventListener('hashchange',()=>{if(location.hash==='#main'){$('#main').focus();return}state.page=1;load();});
...
document.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||b.disabled)return;try{
  ...
  if(b.dataset.confirmReset){b.disabled=true;await request('/api/v2/reset',{method:'POST',body:{scope:b.dataset.confirmReset,confirmation:'DELETE'}});await sync();$('#dialog').close();await load();broadcast();return;}
  switch(b.id){
    ...
    case 'expand':state.expanded=true;$('#main').innerHTML=renderToday(state.briefing);return;
    ...
    case 'refresh':b.disabled=true;try{const d=await request('/api/v2/refresh?'+scope(),{method:'POST',body:{}});$('#main').innerHTML=renderToday(d);}finally{b.disabled=false}return;
    ...
    case 'theme':case 'language':{const changes=b.id==='theme'?{theme:state.preferences.theme==='dark'?'light':'dark'}:{locale:state.preferences.locale==='ko'?'en':'ko'};await request('/api/v2/preferences',{method:'PUT',body:changes});await sync();await load({keep:true});broadcast();return;}
...
document.addEventListener('change',async e=>{if(e.target.id==='daily-date'){state.dailyDate=e.target.value;await loadDaily();$('#daily-date')?.focus();return;}if(!['region','period'].includes(e.target.id))return;const more=$('#load-more');if(more)more.disabled=true;try{if(e.target.id==='region'){state.preferences.region=e.target.value;await request('/api/v2/preferences',{method:'PUT',body:{region:e.target.value}});}else state.period=e.target.value;state.page=1;await load({keep:true});}catch(error){toast(error.message)}});
document.addEventListener('input',e=>{if(e.target.id==='search'){state.q=e.target.value;clearTimeout(searchTimer);searchTimer=setTimeout(async()=>{state.page=1;await load({keep:true});const el=$('#search');el?.focus();},450);}if(e.target.id==='history-search'){const q=e.target.value.toLowerCase();$('#history-list').innerHTML=historyRows(state.history.items.filter(h=>JSON.stringify(h).toLowerCase().includes(q)));}});
