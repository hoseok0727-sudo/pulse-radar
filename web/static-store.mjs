...
const articleUrl=value=>{try{const url=new URL(value);url.hash='';for(const key of [...url.searchParams.keys()])if(/^utm_/i.test(key)||/^(fbclid|gclid)$/i.test(key)||url.hostname==='news.google.com'&&key==='oc')url.searchParams.delete(key);url.searchParams.sort();return url.href}catch{return ''}};
...
function prefs(input={},base=DEFAULT_PREFS){input=input&&typeof input==='object'?input:{};return {categories:list(input.categories??base.categories,CATEGORY_KEYS.length).filter(v=>CATEGORY_KEYS.includes(v)),keywords:list(input.keywords??base.keywords),exclude:list(input.exclude??base.exclude),region:REGION_KEYS.includes(input.region)?input.region:base.region,personalization:typeof input.personalization==='boolean'?input.personalization:base.personalization,onboarded:typeof input.onboarded==='boolean'?input.onboarded:base.onboarded,locale:['ko','en'].includes(input.locale)?input.locale:base.locale,theme:['light','dark'].includes(input.theme)?input.theme:base.theme};}
...
function cleanData(value){
  ...
  const regions=Object.hasOwn(value,'regions')?{}:undefined;
  if(regions)for(const key of REGION_KEYS){const entry=value.regions?.[key];if(!entry||typeof entry!=='object')continue;regions[key]={status:['live','partial-live','unavailable'].includes(entry.status)?entry.status:'unavailable',generatedAt:dateString(entry.generatedAt),sourceGeographies:list(entry.sourceGeographies,12),storyCount:Math.min(100000,Math.max(0,Math.floor(Number(entry.storyCount)||0)))};}
  return {schemaVersion:1,generatedAt:value.generatedAt,...(regions?{regions}:{}),stories:[...new Map(value.stories.map(cleanStory).filter(Boolean).map(s=>[s.topicId,s])).values()],editions:value.editions.filter(e=>e&&e.schemaVersion===1&&realDate(e.date)&&dateString(e.publishedAt)&&e.editorialReview==='primary_sources_checked'&&Array.isArray(e.stories)&&e.stories.length>=3&&e.stories.length<=5).map(e=>({schemaVersion:1,date:e.date,coverageStart:e.coverageStart,coverageEnd:e.coverageEnd,publishedAt:e.publishedAt,headline:string(e.headline,200),headlineEn:string(e.headlineEn,300),intro:localized(e.intro),editorialReview:'primary_sources_checked',stories:e.stories.map(s=>({id:string(s.id,100),category:CATEGORY_KEYS.includes(s.category)?s.category:'society',eventDate:s.eventDate,headline:string(s.headline,200),headlineEn:string(s.headlineEn,300),summary:localized(s.summary),whyItMatters:localized(s.whyItMatters),nextWatch:localized(s.nextWatch),sources:articles(s.sources).map(({title,publisher,url,publishedAt})=>({title,publisher,url,publishedAt}))}))}))};
}
...
export function createStaticRequest({loadData=defaultLoad,storage=browserStorage(),now=()=>Date.now()}={}){
  let memory=emptyState(),lastRaw,storageProblem=!storage,data,pending,loadFailed=false,aliases=new WeakMap();
  ...
  async function published(force=false){if(pending)return pending;if(data&&!force)return data;pending=(async()=>{try{data=cleanData(await loadData({force}));aliases=new WeakMap();loadFailed=false;return data}catch(error){loadFailed=true;if(data)return data;throw error}finally{pending=null}})();return pending;}
  function meta(publication,queryTerms=[],region='korea'){
    const regional=publication.regions?.[region],hasRegions=publication.regions!==undefined,fetchedAt=hasRegions?regional?.generatedAt||null:publication.generatedAt,elapsed=Number(now())-time(fetchedAt),stale=loadFailed||!Number.isFinite(elapsed)||elapsed>36*3600000||elapsed<0,unavailable=hasRegions&&(!regional||regional.status==='unavailable');
    return {mode:unavailable?'offline':stale?'stale-published':regional?.status==='partial-live'?'partial-live':'published',fetchedAt,collection:'published-snapshot',persistence:'browser',region,refreshSupport:'publication-only',storage:storageStatus(),...(regional?{collectionStatus:regional.status,sourceGeographies:regional.sourceGeographies,storyCount:regional.storyCount}:{}),notice:{ko:'운영자가 발행한 공개 자료를 읽고 있습니다. 새로 확인은 최신 발행본을 가져오며, 실시간 웹 검색을 실행하지 않습니다.',en:'You are reading the published collection. Refresh checks for a new publication; it does not run a live web search.'},...(queryTerms.length?{searchQuery:queryTerms.join(', ')}:{})};
  }
  ...
  function publishedStory(snapshot){
    if(!snapshot||!data)return null;const direct=data.stories.find(s=>s.topicId===snapshot.topicId);if(direct)return direct;
    // Public grouping fixes can change an ID. Follow only one unambiguous
    // source match in the same region; the visitor's saved copy stays intact.
    const region=snapshot.region||snapshot.topicId.split('-')[1];if(!REGION_KEYS.includes(region)||!snapshot.topicId.startsWith('public-'+region+'-')||!Array.isArray(snapshot.articles))return null;if(aliases.has(snapshot))return aliases.get(snapshot);
    const urls=new Set(snapshot.articles.map(a=>articleUrl(a.url)).filter(Boolean));
    const matches=data.stories.filter(s=>s.region===region&&s.topicId.startsWith('public-'+region+'-')&&s.articles.some(a=>urls.has(articleUrl(a.url))));
    const current=matches.length===1?matches[0]:null;aliases.set(snapshot,current);return current;
  }
  const sameStory=(record,id)=>record.topicId===id||publishedStory(record.snapshot||record)?.topicId===id;
  function baseStory(id,personal=memory){const direct=data?.stories.find(s=>s.topicId===id);if(direct)return direct;const saved=personal.bookmarks.find(b=>b.topicId===id)?.snapshot||personal.briefings.flatMap(b=>b.items).find(s=>s.topicId===id)||personal.feedback.find(f=>f.topicId===id)?.snapshot||null;return publishedStory(saved)||saved;}
  function decorate(raw,personal,{baseline=[],baselineAt=null,personalized=true}={}){
    const p=personal.preferences,active=personalized&&p.personalization!==false,marks=personal.feedback.filter(f=>sameStory(f,raw.topicId)),matchedKeywords=active?p.keywords.filter(k=>storyMatches(raw,k)):[],followed=personal.bookmarks.some(b=>sameStory(b,raw.topicId)&&b.kind==='follow'),saved=personal.bookmarks.some(b=>sameStory(b,raw.topicId)&&b.kind==='save'),read=marks.some(f=>f.kind==='read'&&f.fingerprint===raw.fingerprint),prior=baseline.find(s=>sameStory(s,raw.topicId)),old=new Set(prior?.articles.map(a=>articleUrl(a.url))||[]),newArticles=prior?raw.articles.filter(a=>!old.has(articleUrl(a.url))&&(!baselineAt||time(a.publishedAt)>=time(baselineAt))):[];
    const change=!baselineAt?{kind:'first',ko:'처음 살펴보는 이야기입니다.',en:'Your first look at this story.',articles:[]}:!prior?time(raw.publishedAt)>time(baselineAt)?{kind:'new',ko:'지난 브리핑 이후 발행된 이야기입니다.',en:'Published since your previous briefing.',articles:[]}:{kind:'first',ko:'이번 브리핑에서 처음 고른 이야기입니다. 원문은 이전 방문 전에 발행됐습니다.',en:'New to this selection, with sources published before your previous visit.',articles:[]}:newArticles.length?{kind:'coverage',ko:`지난번 이후 발행 자료에 보도 ${newArticles.length}건이 더해졌습니다. 새로운 사실인지는 원문을 확인해 주세요.`,en:`${newArticles.length} reports were added to the published collection. Read the originals to check for new facts.`,articles:newArticles.slice(0,3)}:{kind:'unchanged',ko:'지난번과 같은 발행 자료입니다. 다음 발행본에서 후속 보도를 확인할 수 있습니다.',en:'The same published coverage as last time. A later edition may contain follow-up reports.',articles:[]};
  ...
  return async function request(path,{method='GET',body}={}){
    ...
    let personal=read();const scope={region:REGION_KEYS.includes(url.searchParams.get('region'))?url.searchParams.get('region'):'korea',period:PERIODS.includes(url.searchParams.get('period'))?url.searchParams.get('period'):'day'};
    ...
    if(verb==='GET'&&route.startsWith('/api/v2/story/')){await published().catch(error=>{personal=read();if(!personal.bookmarks.length&&!personal.briefings.length&&!personal.feedback.length)throw error});personal=read();let id;try{id=decodeURIComponent(route.slice('/api/v2/story/'.length))}catch{throw fail('이야기 주소를 확인해 주세요.')}const story=baseStory(id,personal);if(!story)throw fail('발행 자료와 내 보관함에서 이야기를 찾지 못했습니다.',404);return decorate(story,personal);}
    if(verb==='POST'&&route==='/api/v2/action'){
      if(typeof body?.value!=='boolean'||!['save','follow','read','useful','hide'].includes(body?.kind))throw fail('변경할 기록과 상태를 확인해 주세요.');await published().catch(error=>{if(!baseStory(body.topicId,read()))throw error});
