...
export function createStaticRequest({loadData=defaultLoad,storage=browserStorage(),now=()=>Date.now()}={}){
  ...
  function publishedStory(snapshot){
    ...
    if(!/^public-(korea|world)-/.test(snapshot.topicId)||!Array.isArray(snapshot.articles))return null;if(aliases.has(snapshot))return aliases.get(snapshot);
    const region=snapshot.region||snapshot.topicId.split('-')[1],urls=new Set(snapshot.articles.map(a=>articleUrl(a.url)).filter(Boolean));
  ...
  return async function request(path,{method='GET',body}={}){
    ...
    let personal=read();const scope={region:url.searchParams.get('region')==='world'?'world':'korea',period:['day','week','month'].includes(url.searchParams.get('period'))?url.searchParams.get('period'):'day'};
